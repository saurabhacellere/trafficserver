...
Http1ServerSession::new_connection(NetVConnection *new_vc)
{
  ...
  Debug("http_ss", "[%" PRId64 "] session born, netvc %p", connection_id(), new_vc);
  state = HSS_INIT;
...
Http1ServerSession::enable_outbound_connection_tracking(OutboundConnTrack::Group *group)
{
  ...
  if (is_debug_tag_set("http_ss")) {
    ...
    w.print("[{}] new connection, ip: {}, group ({}), count: {}\0", connection_id(), get_server_ip(), *group, group->_count);
...
Http1ServerSession::do_io_close(int alerrno)
{
  ...
  if (debug_p) {
    w.print("[{}] session close: nevtc {:x}", connection_id(), server_vc);
  }
  ...
  if (conn_track_group) {
    if (conn_track_group->_count >= 0) {
      ...
      Error("[http_ss] [%" PRId64 "] number of connections should be greater than or equal to zero: %u", connection_id(),
