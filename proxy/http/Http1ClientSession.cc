...
#define STATE_ENTER(state_name, event, vio)                                                                      \
  do {                                                                                                           \
    ...
    HttpSsnDebug("[%" PRId64 "] [%s, %s]", connection_id(), #state_name, HttpDebugNames::get_event_name(event)); \
  } while (0)
...
Http1ClientSession::destroy()
{
  ...
  if (!in_destroy) {
    ...
    HttpSsnDebug("[%" PRId64 "] session destroy", connection_id());
...
Http1ClientSession::new_connection(NetVConnection *new_vc, MIOBuffer *iobuf, IOBufferReader *reader)
{
  ...
  _connection_id = ProxySession::next_connection_id();
  ...
  HttpSsnDebug("[%" PRId64 "] session born, netvc %p", connection_id(), new_vc);
...
Http1ClientSession::do_io_close(int alerrno)
{
  ...
  if (half_close && this->trans.get_sm()) {
    ...
    HttpSsnDebug("[%" PRId64 "] session half close", connection_id());
    ...
    HttpSsnDebug("[%" PRId64 "] session closed", connection_id());
...
Http1ClientSession::release(ProxyTransaction *trans)
{
  ...
  if (more_to_read) {
    trans->destroy();
    HttpSsnDebug("[%" PRId64 "] data already in buffer, starting new transaction", connection_id());
    new_transaction();
  } else {
    HttpSsnDebug("[%" PRId64 "] initiating io for next header", connection_id());
...
Http1ClientSession::attach_server_session(Http1ServerSession *ssession, bool transaction_done)
{
  if (ssession) {
    ...
    bound_ss        = ssession;
    HttpSsnDebug("[%" PRId64 "] attaching server session [%" PRId64 "] as slave", connection_id(), ssession->connection_id());
