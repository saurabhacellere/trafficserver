...
NextHopConsistentHash::Init(const YAML::Node &n)
{
  ...
  for (uint32_t i = 0; i < groups; i++) {
    ...
    for (uint32_t j = 0; j < hosts[i].size(); j++) {
      // ATSConsistentHash needs the raw pointer.
      HostRecord *p = hosts[i][j].get();
      ...
      p->group_index = hosts[i][j]->group_index;
      p->host_index  = hosts[i][j]->host_index;
      hash_ring->insert(p, p->weight, &hash);
      NH_Debug("next_hop", "Loading hash rings - ring: %d, host record: %d, name: %s, hostname: %s", i, j, p->name,
               p->hostname.c_str());
    }
    hash.clear();
...
NextHopConsistentHash::getHashKey(uint64_t sm_id, HttpRequestData *hrdata, ATSHash64 *h)
{
  ...
  switch (hash_key) {
  // TODO - figure out what the difference is between a URL and a URI in the context
  // of creating a hash key from a http request.  In the meantime, URI and URL are
  // the same.
  case NH_URI_HASH_KEY:
  case NH_URL_HASH_KEY:
    url_string_ref = url->string_get_ref(&len, true);
    if (url_string_ref && len > 0) {
      h->update(url_string_ref, len);
      NH_Debug("next_hop", "[%" PRId64 "] url hash string: %s", sm_id, url_string_ref);
    }
    h->final();
    hash_value = h->get();
    break;
  ...
  case NH_CACHE_HASH_KEY:
    ...
    if (ps_url) {
      ...
      if (url_string_ref && len > 0) {
        NH_Debug("next_hop", "[%" PRId64 "] using parent selection over-ride string:'%.*s'.", sm_id, len, url_string_ref);
        h->update(url_string_ref, len);
      }
    } else {
      url_string_ref = url->path_get(&len);
      h->update("/", 1);
      if (url_string_ref && len > 0) {
        NH_Debug("next_hop", "[%" PRId64 "] the parent selection over-ride url is not set, using default path: %s.", sm_id,
                 url_string_ref);
        h->update(url_string_ref, len);
      }
    }
                                   ...
                                   const uint64_t retry_time)
{
  ...
  if (firstcall) {
    NH_Debug("next_hop", "[%" PRId64 "] firstcall, line_number: %d, result: %s", sm_id, result->line_number,
    ...
    NH_Debug("next_hop", "[%" PRId64 "] not firstcall, line_number: %d, result: %s", sm_id, result->line_number,
             ParentResultStr[result->result]);
    switch (ring_mode) {
  ...
  do { // search until we've selected a different parent if !firstcall
    ...
    lookups++;
    // the 'available' flag is maintained in 'hosts' and not the hash ring.
    if (hostRec) {
      pRec = hosts[hostRec->group_index][hostRec->host_index];
      if (firstcall) {
  ...
  NH_Debug("next_hop", "[%" PRId64 "] Initial parent lookups: %d", sm_id, lookups);
    ...
    do {
      ...
      if (pRec && !pRec->available && host_stat == HOST_STATUS_UP) {
        ...
        if ((pRec->failedAt + retry_time) < static_cast<unsigned>(now)) {
          ...
          NH_Debug("next_hop", "[%" PRId64 "] next hop %s is now retryable, marked it available.", sm_id, pRec->hostname.c_str());
          break;
        }
      }
      switch (ring_mode) {
      ...
      if (hostRec) {
        ...
        NH_Debug("next_hop", "[%" PRId64 "] Selected a new parent: %s, available: %s, wrapped: %s, lookups: %d.", sm_id,
      ...
      if (all_wrapped) {
        NH_Debug("next_hop", "[%" PRId64 "] No available parents.", sm_id);
        if (pRec) {
          pRec = nullptr;
        }
        break;
