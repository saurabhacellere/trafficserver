...
LogAccess::marshal_client_sni_server_name(char *buf)
{
  ...
  // the length.
  //
  std::string_view sn = "n/a";

  if (m_http_sm && !m_http_sm->client_sni_server_name().empty()) {
    sn = std::string_view(m_http_sm->client_sni_server_name().c_str(), m_http_sm->client_sni_server_name().length());
  }
