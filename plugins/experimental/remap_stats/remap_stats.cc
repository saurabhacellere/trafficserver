...
static void
stat_add(const std::string &name, TSMgmtInt amount, TSStatPersistence persist_type, TSMutex create_mutex)
{
  int stat_id = -1;
  ...
  if (unlikely(hash.find(name) == hash.cend())) {
    ...
    if (TS_ERROR == TSStatFindName(name.c_str(), &stat_id)) {
      stat_id = TSStatCreate(name.c_str(), TS_RECORDDATATYPE_INT, persist_type, TS_STAT_SYNC_SUM);
      if (stat_id == TS_ERROR) {
        TSDebug(DEBUG_TAG, "Error creating stat_name: %s", name.c_str());
      } else {
        TSDebug(DEBUG_TAG, "Created stat_name: %s stat_id: %d", name.c_str(), stat_id);
      }
    }
    ...
    if (stat_id >= 0) {
      ...
      TSDebug(DEBUG_TAG, "Cached stat_name: %s stat_id: %d", name.c_str(), stat_id);
    }
  } else {
  ...
  if (likely(stat_id >= 0)) {
    ...
    TSDebug(DEBUG_TAG, "stat error! stat_name: %s stat_id: %d", name.c_str(), stat_id);
  }
}
...
static std::string
create_stat_name(const std::string &h, const std::string &b)
{
  std::string res(std::string("plugin.") + PLUGIN_NAME + '.' + h + '.' + b);
  return res.substr(0, MAX_STAT_LENGTH);
}
...
handle_txn_close(TSCont cont, TSEvent event ATS_UNUSED, void *edata)
{
  ...
  hostname = std::string(reinterpret_cast<char *>(reinterpret_cast<uintptr_t>(txnd) & ~0x01)); // Get hostname
  ...
  if (txnd) {
    if (reinterpret_cast<uintptr_t>(txnd) & 0x01) // remap succeeded?
    {
      ...
      create_stat_name(remap, "in_bytes");
      stat_add(stat_name, static_cast<TSMgmtInt>(in_bytes), config->persist_type, config->stat_creation_mutex);

      out_bytes = TSHttpTxnClientRespHdrBytesGet(txn);
      out_bytes += TSHttpTxnClientRespBodyBytesGet(txn);

      create_stat_name(remap, "out_bytes");
      stat_add(stat_name, static_cast<TSMgmtInt>(out_bytes), config->persist_type, config->stat_creation_mutex);
      ...
      if (TSHttpTxnClientRespGet(txn, &buf, &hdr_loc) == TS_SUCCESS) {
        ...
        if (status_code < 200) {
          create_stat_name(remap, "status_other");
        } else if (status_code <= 299) {
          create_stat_name(remap, "status_2xx");
        } else if (status_code <= 399) {
          create_stat_name(remap, "status_3xx");
        } else if (status_code <= 499) {
          create_stat_name(remap, "status_4xx");
        } else if (status_code <= 599) {
          create_stat_name(remap, "status_5xx");
        } else {
          create_stat_name(remap, "status_other");
        }
        ...
        create_stat_name(remap, "status_unknown");
        stat_add(stat_name, 1, config->persist_type, config->stat_creation_mutex);
      }
    }
  }
