...
The **hosts** definitions is a **YAML** map of hosts.  This map is **optional** but if not used, the
**groups** list **must** include complete defintions for hosts.  See the **group** examples below.

In the example below, **hosts** is a **YAML** map of hosts.  The map **keys** are **p1** and **p2**
and eache key uses a **YAML** anchor, **&p1** and **&p2** that may be used elsewhere in the **YAML** document to refer to hosts **p1** and **p2**.

- **host**: the host key value is a hostname string
- **protocol**: a list of protocols the hosts supports, **http** or **https**
- **healthcheck**: health check information with the **url** used to check 
  the hosts health by some external healt check agent.
...
When using and **references**, the complete **YAML** document must include the **anchors** portion of the document first.
...
- **hash_key**: The hashing key used by the **consistent_hash** policy. If not specified, defaults to **path**. Use one of::
...
- **go_direct** - A boolean value indicating wether a transaction may bypass proxies and go direct to the origin. Defaults to **true**
...
- **protocol**: Indicates which protocol the strategy supports, *http* or *https*
